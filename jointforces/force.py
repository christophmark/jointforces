    ...
    for t in tqdm(range(len(angles[0][:n_max]))):                   
        ...
        if save_plot:
            ...
            if extra_large_pressure == True:
                # now use 100 as maximum
                x = [np.cos(i*np.pi/180) for i in angle_list] * (pressures / np.array([2000]*len(pressures)) ) 
                y = [np.sin(i*np.pi/180) for i in angle_list] * (pressures / np.array([2000] *len(pressures)) ) 
				# combined figure
				# plot circles
                circle_a = plt.Circle((0, 0), 500/ 2000 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                circle_b = plt.Circle((0, 0), 1000/ 2000 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                circle_c = plt.Circle((0, 0), 1500/ 2000 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                ax1.text(0.5, 0.89, '1500Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                ax1.text(0.5, 0.643, '500Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                ax1.text(0.5, 0.77, '1000Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                ax1.add_artist(circle_a)
                ax1.add_artist(circle_b)
                ax1.add_artist(circle_c)
                ax1.axis('equal')  
