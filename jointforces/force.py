...
def reconstruct_gui(result, lookupfile, muperpixel, r_min=2, angle_filter=20, r_max=None,  continuous_radii = False):
    """
    - folder: contains PIV results
    - lookupfile: Material look-up table for force reconstruction
    - muperpixel: Pixelsize
    ...
    # initial spheroid radius and surface (used for force estimation)
    r0 = result.segmentations[0]['radius']
    A0 = 4 * np.pi * (r0 * (10 ** -6)) ** 2
    ...
    # loop over series of PIV results
    for i in range(len(result.displacements)):
        dis = result.displacements[i]
        if len(result.segmentations) == 1:
            seg = result.segmentations[0]
        else:
            seg = result.segmentations[i]
...
def angle_analysis(folder, output, n_max=None,save_plot=True, fontsize=7, name_of_resultfile='result_angles_pressure.xlsx', dt=None, angle_legend=False, 
                   path_of_resultfile=None,small_pressure = False, medium_pressure=False, large_pressure=False, extra_large_pressure = False, extra_extra_large_pressure=False):
    """
    ...
    for t in tqdm(range(len(angles[0][:n_max]))):                   
        ...
        if save_plot:
            ...
            if medium_pressure == True:	
				# now use 1000 as maximum
                x = [np.cos(i*np.pi/180) for i in angle_list] * (pressures / np.array([1500]*len(pressures)) ) 
                y = [np.sin(i*np.pi/180) for i in angle_list] * (pressures / np.array([1500] *len(pressures)) ) 
				# combined figure
				# plot circles
                circle_1000pa = plt.Circle((0, 0), 1000/ 1500 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                circle_250pa = plt.Circle((0, 0), 250/ 1500 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                circle_500pa = plt.Circle((0, 0), 500/ 1500 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                circle_750pa = plt.Circle((0, 0), 750/ 1500 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                ax1.text(0.5, 0.855, '1000 Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                ax1.text(0.5, 0.685, '500 Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                ax1.text(0.5, 0.605, '250 Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                ax1.text(0.5, 0.775, '750 Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                ax1.add_artist(circle_1000pa)
                ax1.add_artist(circle_500pa)
                ax1.add_artist(circle_250pa)
                ax1.add_artist(circle_750pa)
                ax1.axis('equal')  
...
# 				 transform = ax1.transAxes, fontsize=9.2)  
                ...
                circle_500pa = plt.Circle((0, 0), 500/ 1500 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                circle_750pa = plt.Circle((0, 0), 750/ 1500 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                axb.text(0.5, 0.859, '1000 Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=axb.transAxes,fontsize=fontsize)
                axb.text(0.5, 0.689, '500 Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=axb.transAxes,fontsize=fontsize)
                axb.text(0.5, 0.609, '250 Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=axb.transAxes,fontsize=fontsize)
                axb.text(0.5, 0.779, '750 Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=axb.transAxes,fontsize=fontsize)
                axb.add_artist(circle_1000pa)
                axb.add_artist(circle_250pa)
                axb.add_artist(circle_500pa)
                axb.add_artist(circle_750pa)   
                axb.axis('equal')  
            ...
            if large_pressure == True:
                ...
                # now use 100 as maximum
                x = [np.cos(i*np.pi/180) for i in angle_list] * (pressures / np.array([2500]*len(pressures)) ) 
                y = [np.sin(i*np.pi/180) for i in angle_list] * (pressures / np.array([2500] *len(pressures)) ) 
                # combined figure
                # plot circles
                circle_a = plt.Circle((0, 0), 500/ 2500 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                circle_b = plt.Circle((0, 0), 1000/ 2500 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                circle_c = plt.Circle((0, 0), 1500/ 2500 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                ax1.text(0.5, 0.82, '1500Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                ax1.text(0.5, 0.62, '500Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                ax1.text(0.5, 0.72, '1000Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                ax1.add_artist(circle_a)
                ax1.add_artist(circle_b)
                ax1.add_artist(circle_c)
                ax1.axis('equal')  
                ...
                circle_b = plt.Circle((0, 0), 1000/ 2500 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                circle_c = plt.Circle((0, 0), 1500/ 2500 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                axb.text(0.5, 0.827, '1500Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=axb.transAxes,fontsize=fontsize)
                axb.text(0.5, 0.627, '500Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=axb.transAxes,fontsize=fontsize)
                axb.text(0.5, 0.727, '1000Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=axb.transAxes,fontsize=fontsize)
                axb.add_artist(circle_a)
                axb.add_artist(circle_b)
                axb.add_artist(circle_c)
                axb.axis('equal')  
            ...
            if extra_large_pressure == True:
                ...
                circle_b = plt.Circle((0, 0), 2000/ 4000 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                circle_c = plt.Circle((0, 0), 3000/ 4000 , color=colorcycle, zorder=10, fill=False, linestyle='--')
                # axb.text(0.62, 0.99, '3000Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                # axb.text(0.62, 0.81, '1000Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                # axb.text(0.62, 0.97, '2000Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes,fontsize=fontsize)
                axb.text(0.5, 0.9, '3000Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=axb.transAxes,fontsize=fontsize)
                axb.text(0.5, 0.65, '1000Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=axb.transAxes,fontsize=fontsize)
                axb.text(0.5, 0.78, '2000Pa', color = colorcycle ,horizontalalignment='center', verticalalignment='center', transform=axb.transAxes,fontsize=fontsize)
                axb.add_artist(circle_a)
                axb.add_artist(circle_b)
                axb.add_artist(circle_c)
                axb.axis('equal')  
